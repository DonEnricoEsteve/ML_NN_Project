...
#                          conds=base_conds, tmin=component_start_times[4], tmax=component_end_times[4]) 
...
PPC_directory = f"{wd}/BL_15-5-10/Evoked_PPC"
...
preprocess_helper(input_dir=npy_directory, output_dir=PPC_directory, 
                  process_func=calculate_pseudo_trials_with_pca, 
                  n_groups=10, n_components=None, suffix='dec_pseudo_PCA')
...
perform_general_decoding(base_dir=PPC_directory, tasks=binary_tasks, 
                         classification_type='binary', output_filename="BL_15-5-10")

# Run multi-class classification (n=9 tasks)
# Define output filename (recommended convention is window_binsize_groups_components)
perform_general_decoding(base_dir=PPC_directory, tasks=multiclass_tasks, 
                         classification_type='multi', output_filename="BL_15-5-10")

# Load the resulting lists (from general decoding) of within-subject accuracies for each task (n=18)
list_binary_accuracies = np.load(f"{results_dir}/list_binary_accuracies_BL_15-5-10.npy")
list_multi_accuracies = np.load(f"{results_dir}/list_multi_accuracies_BL_15-5-10.npy")
